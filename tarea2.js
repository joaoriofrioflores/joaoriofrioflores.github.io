...
function calcularTotal(){
    ...
    for(var i=1, fila; fila = tabla.rows[i]; i++) {        
        if( document.getElementById("chk_" + i).checked ){            
            console.log("Seleccionado Fila "  + i +  " con precio => " + fila.cells[1].innerHTML) +  ",cantida => " + document.getElementById(i).value
        + "<h3> SUB - TOTAL =" + (FILA.CELLS[1].innerHTML * document.getElementById(i).value
     TOTAL *=(fila.cells[1].innerHTML * document.getElementById(i).value)   
        }
    }
document.getElementById(pago).innerHTML += "<h1> TOTAL DE VENTA =" + TOTAL </h1>
A += 2; A => A + 2
A -= 2; A => A - 2
